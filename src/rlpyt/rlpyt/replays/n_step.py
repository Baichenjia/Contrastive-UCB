...
class BaseNStepReturnBuffer(BaseReplayBuffer):
    ...
    *not* be bootstrapped.
    ...
    def compute_returns(self, T):
        ...
        #print("\ncompute returns:", self.n_step_return)
        #print(self.t, T, self.T)
        #print(self.samples.reward.shape, self.samples.action.shape, self.samples.done.shape)
        # self.samples.reward.shape = self.samples.action.shape = self.samples.done.shape  = (1000000, 1)
        if self.n_step_return == 1:
            return  # return = reward, done_n = done
        # T = 1, self.T = 1000000
        t, s = self.t, self.samples      # t 每次迭代是增加的, samples is a dict with action, reward, done
        nm1 = self.n_step_return - 1
        if t - nm1 >= 0 and t + T <= self.T:  # No wrap (operate in-place).
            reward = s.reward[t - nm1:t + T]   # length = 10
            done = s.done[t - nm1:t + T]    
            #print("reward:", reward.shape, t, nm1, T)
            return_dest = self.samples_return_[t - nm1: t - nm1 + T]
            #print("%.10f" %return_dest[0][0])
