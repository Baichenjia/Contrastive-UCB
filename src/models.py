...
class SPRCatDqnModel(torch.nn.Module):
	...
	def __init__(
		...
		self.jumps = jumps
		self.model_rl = model_rl                          # 0.0
		self.use_spr = spr                                # 1.0
		self.target_augmentation = target_augmentation    # 1
		self.eval_augmentation = eval_augmentation        # 0
		self.num_actions = output_size

		if dueling:     # use this network (default)
		...
		if self.jumps > 0:   # jumps = 5 by default
	...
	def stem_forward(self, img, prev_action=None, prev_reward=None):
		...
		# img.shape = (None, 4, 84, 84)
		lead_dim, T, B, img_shape = infer_leading_dims(img, 3)   # [1, 1, None, (4, 84, 84)]

		conv_out = self.conv(img.view(T * B, *img_shape))  # Fold if T dimension.  (None, 64, 7, 7)
		if self.renormalize:
	...
	def forward(self, observation,
		...
		if train:
			...
			pred_reward = []
			pred_latents = []
			# observation.shape=[16, 32, 4, 1, 84, 84], input_obs.shape=[32, 4, 84, 84]
			input_obs = observation[0].flatten(1, 2)

			input_obs = self.transform(input_obs, augment=True)
			latent = self.stem_forward(input_obs,
									   prev_action[0],
									   prev_reward[0])        # latent.shape=(32, 64, 7, 7)
			# Q-network prediction  shape=(32, num_action, 51)
			log_pred_ps.append(self.head_forward(latent,
												 prev_action[0],
												 prev_reward[0],
												 logits=True))
			pred_latents.append(latent)     # dynamic model prediction
			if self.jumps > 0:              # self.jumps = 5
				...
				for j in range(1, self.jumps + 1):
					latent, pred_rew = self.step(latent, prev_action[j])   # use dynamic model
					pred_rew = pred_rew[:observation.shape[1]]
					pred_latents.append(latent)
					pred_reward.append(F.log_softmax(pred_rew, -1))

			if self.model_rl > 0:    # , self.model_rl = 0.0
...
class DQNDistributionalDuelingHeadModel(torch.nn.Module):
	...
	def __init__(self,
				 ...
				 std_init=0.1):
		super().__init__()
		print("in DQNDistributionalDuelingHeadModel:", input_channels, output_size, pixels, n_atoms, hidden_size, noisy)
		if noisy:
...
class TransitionModel(nn.Module):
	def __init__(self,
		...
		self.network = nn.Sequential(*layers)
		self.reward_predictor = RewardPredictor(channels, pixels=pixels, limit=limit, norm_type=norm_type)
		self.train()
